...
namespace hihapi.Controllers
{
    ...
    public class FinanceDocumentTypesController : ODataController
    {
        ...
        public async Task<IActionResult> Post([FromBody] FinanceDocumentType ctgy)
        {
            ...
            var hms = await _context.HomeMembers.Where(p => p.HomeID == ctgy.HomeID.Value && p.User == usrName).CountAsync();
            if (hms <= 0)
            {
                throw new UnauthorizedAccessException();
            }
        ...
        public async Task<IActionResult> Put([FromODataUri] int key, [FromBody] FinanceDocumentType update)
        {
            ...
            var existing = await _context.FinDocumentTypes.FindAsync((short)key);
            if (existing == null)
            {
                return NotFound();
            }
            ...
            // Reject HomeID changes via PUT (incl. null <-> non-null)
